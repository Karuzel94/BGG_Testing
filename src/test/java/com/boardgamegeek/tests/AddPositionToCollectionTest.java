...
public class AddPositionToCollectionTest extends BaseTest {
    ...
    public void addPositionToCollectionTest() {
        ...
        Log.logInfo("The saved String of added game '"
                + gameName + "' is equal to the : '" + gamePage.getGameTitle()
                + "' game name in collection");
        assertThat(gamePage.getInformationIsGameAdded()).isEqualTo("In Collection");
        Log.logInfo("Game is added to collection, the Adding Button is changed to : "
                + gamePage.getInformationIsGameAdded());
    }
